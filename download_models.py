...
def download_hy_worldplay():
    ...
    if os.path.exists(model_src) and not os.path.exists(model_dst):
        ...
        print("Fixed: Renamed model.safetensors -> diffusion_pytorch_model.safetensors in ar_distilled_action_model")
...
def download_llm_text_encoder(hunyuan_path):
    ...
    if os.path.exists(llm_target) and os.path.isdir(llm_target) and len(os.listdir(llm_target)) > 5:
...
def download_byt5_encoders(hunyuan_path):
    ...
    if os.path.exists(byt5_target) and os.path.isdir(byt5_target) and len(os.listdir(byt5_target)) > 3:
    ...
    if os.path.exists(glyph_target) and os.path.exists(os.path.join(glyph_target, 'checkpoints', 'byt5_model.pt')):
...
def download_vision_encoder(hunyuan_path, hf_token):
    ...
    if not hf_token:
        ...
        print("The vision encoder requires access to: https://huggingface.co/black-forest-labs/FLUX.1-Redux-dev")
        print("Skipping vision encoder download.")
        print("\nYou can download it manually later with:")
        print("  huggingface-cli download black-forest-labs/FLUX.1-Redux-dev --local-dir <path>/vision_encoder/siglip --token <your_token>")
        return
    ...
    if os.path.exists(siglip_target) and os.path.isdir(siglip_target) and len(os.listdir(siglip_target)) > 3:
    ...
    try:
        ...
        print("Make sure you have requested access to FLUX.1-Redux-dev and your token is valid.")
...
def print_paths():
    ...
    print(f"AR_ACTION_MODEL_PATH={worldplay_path}/ar_model/diffusion_pytorch_model.safetensors")
    print(f"BI_ACTION_MODEL_PATH={worldplay_path}/bidirectional_model/diffusion_pytorch_model.safetensors")
    print(f"AR_DISTILL_ACTION_MODEL_PATH={worldplay_path}/ar_distilled_action_model/diffusion_pytorch_model.safetensors")
